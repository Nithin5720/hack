...
import { Mail, Loader2, Send, Edit, RefreshCw, CheckCircle, ShieldAlert, Zap, Cpu, Activity, User, Tag, Inbox, FileEdit, Trash2, PenBox, X } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col z-30">
        ...
        <div className="p-4">
          <button onClick={() => setIsComposeOpen(true)} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-bold shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all">
      ...
      {isComposeOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700/50 w-full max-w-lg rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col">
            <div className="p-4 bg-slate-800/50 border-b border-slate-700/50 flex justify-between items-center">
              <h3 className="font-bold tracking-wide text-slate-200 flex items-center gap-2">
                <PenBox className="w-4 h-4 text-blue-400" /> New Message
              </h3>
            ...
            <div className="p-5 flex flex-col gap-4">
              <input 
                ...
                className="w-full bg-slate-950/50 border border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500 transition-colors placeholder:text-slate-500"
              />
              <input 
                type="text"
                placeholder="Subject:"
              ...
              <textarea 
                ...
                className="w-full h-48 bg-slate-950/50 border border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-blue-500 transition-colors resize-none placeholder:text-slate-500 custom-scrollbar"
              />
            </div>
            <div className="p-4 bg-slate-800/30 border-t border-slate-700/50 flex justify-end">
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-lg font-bold text-sm tracking-wide shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all flex items-center gap-2 disabled:opacity-50"
              >
                {isSendingCompose ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
